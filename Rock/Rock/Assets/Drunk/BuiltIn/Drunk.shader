	...
	SubShader {
		...
		Pass {   // pass 1 - rotate view
			...
			float4 frag (v2f_img input) : SV_TARGET
			{
				...
				float4 shifted = tex2D(_MainTex, input.uv + offset);
				float4 orig = tex2D(_MainTex, input.uv);
		...
		Pass {   // pass 2 - split view
			...
			float4 frag (v2f_img input) : SV_TARGET
			{
				...
				float4 tc = tex2D(_MainTex, input.uv);
				float4 tl = tex2D(_MainTex, input.uv - float2(sin(sq), 0));
				float4 tR = tex2D(_MainTex, input.uv + float2(sin(sq), 0));
				float4 tD = tex2D(_MainTex, input.uv - float2(0, sin(sq)));
				float4 tU = tex2D(_MainTex, input.uv + float2(0, sin(sq)));
		...
		Pass {   // pass 4 - distort with noise
			...
			float4 frag (v2f_img i) : SV_TARGET
			{
				...
				float n = snoise(float3((i.uv * _Frequency), _Period + _Time.y)) * PI;
		...
		Pass {   // pass 7 - deform
			...
			half4 frag (v2f_img input) : SV_TARGET
			{
				...
				return tex2D(_MainTex, waved_uv);
			}
			ENDCG
		}
	}
